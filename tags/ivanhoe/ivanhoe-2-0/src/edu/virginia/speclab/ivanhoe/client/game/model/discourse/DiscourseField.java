 ...
 * @author lfoster
 * 
 * <code>DiscourseField</code> is a map of all the documents and moves by all
...
public class DiscourseField implements IMessageHandler, IRoleListener {
	...
	private List discourseFieldListeners, permissionListeners;
	...
	private static class OpenDocumentRecord {
		...
		public void addRef() {
			SimpleLogger.logInfo("Adding reference to "
					+ this.document.getVersion().toString());
			this.referenceCount++;
		}

		public void release() {
			SimpleLogger.logInfo("Reference to "
					+ this.document.getVersion().toString() + " released");
			this.referenceCount--;
		}
	}
	...
	private void firePermissionsChanged() {
		...
		for (Iterator i = permissionListeners.iterator(); i.hasNext();) {
			IPermissionListener permissionListener = (IPermissionListener) i
					.next();
	...
	/**
	 * Returns a discourse field timeline which contains all moves by all
	 * players sorted chronologically.
	 * 
	 * @return
	 */
	 ...
	 * @param userName
	 *            The owner of the move history to use.
	 * @param listener
	 *            A listener for when the document is done loading.
	 */
	public void requestDocument(DocumentVersion version,
			IDocumentLoaderListener listener) {
		...
		if (doc != null) {
			SimpleLogger
					.logInfo("Document already loaded, using existing document.");
			addReference(version);
			listener.documentLoaded(doc);
			return;
		}

		DocumentLoader loader = new DocumentLoader(this, this.documentCache,
				version, listener, isReadOnly(version));

		loader.start();
	}

	/**
	 * Check if a version is read-only for this player. Rules: 1) If the
	 * DiscourseField is read-only, all docs are read-only 2) Otherwise, it's
	 * writable if the DocumentVersion is newly created
	 * 
	 * @param version
	 * @return
	 */
	...
	/**
	 * Obtain a list of all the documents currently available in the discourse
	 * field, including documents that have not yet been opened by the player in
	 * this game.
	 * 
	...
	/**
	 * Adds an image as a supporting file to the specified document. The call is
	 * passed through to the cache
	 * 
	 * @param info
	 *            Document that will own the image
	 * @param imageFile
	 *            Image file
	 */
	...
	/**
	 * handle messages from the server This is used to get the list of documents
	 * and their data
	 */
	...
	public List getActionList() {
		...
		for (Iterator i = moveHistory.iterator(); i.hasNext();) {
			...
			for (Iterator j = playerHistory.playerMoveHistory.iterator(); j
					.hasNext();) {
	...
	public void addMoveToHistory(Move move) {
		...
		if (history == null) {
			...
			SimpleLogger
					.logInfo("Creating history for player [" + player + "]");
	...
	/**
	 * This function is called by the document loader to inform the discourse
	 * field of the presence of a new document which must be kept up to date.
	 * 
	 * @param doc
	 */
	public void addOpenDocument(IvanhoeDocument doc) {
		SimpleLogger.logInfo("Adding open document record for "
				+ doc.getVersion().toString());
		this.openDocuments.add(new OpenDocumentRecord(doc));
	}

	/**
	 * Remove document reference from open document list Notify current move
	 * that the document is no longer open
	 * 
	 * @param document
	 */
	public void removeReference(IvanhoeDocument document) {
		...
		for (Iterator itr = this.openDocuments.iterator(); itr.hasNext();) {
			...
			if (rec.document.equals(document)) {
				...
				if (rec.isOpen() == false) {
					SimpleLogger
							.logInfo("This is the last ref to the doc, removing record.");
					...
					final DocumentVersion parentVersion = docVersion
							.getParentVersion();

					// If there's no parent, then don't remove it
					if (parentVersion != null) {
						final Collection currentActions = docVersion
								.getActionIDs();
						currentActions.removeAll(parentVersion.getActionIDs());
						if (currentActions.isEmpty()) {
							documentVersionManager
									.removeDocumentVersion(docVersion);
						}
					}
	 ...
	 * @param userName
	 *            the name of a role
	 * @return a list of moves by player userName
	 */
	...
	/**
	 * Return a cloned instance of the action identified by
	 * <code>actionId</code>
	 * 
	 * @param actionId
	 * @return
	...
	public IvanhoeAction lookupAction(String actionId) {
		...
		if (act == null) {
			SimpleLogger.logError("Unable to find action " + actionId
					+ " in DF");
		}
